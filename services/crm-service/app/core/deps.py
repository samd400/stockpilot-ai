...
def require_crm_role(user: Dict = Depends(get_current_user)) -> Dict:
    if user["role"] not in {"admin", "manager", "crm_staff"}:
        raise HTTPException(status_code=403, detail="CRM staff, manager, or admin required")
    return user

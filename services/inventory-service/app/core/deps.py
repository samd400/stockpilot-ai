...
def require_inventory_role(user: Dict = Depends(get_current_user)) -> Dict:
    if user["role"] not in INVENTORY_ROLES:
        raise HTTPException(status_code=403, detail="Inventory staff, manager, or admin required")
    return user
